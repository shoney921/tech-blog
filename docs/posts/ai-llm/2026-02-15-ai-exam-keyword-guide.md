---
title: "3000명 인사고과 AI 시험을 준비하며 정리한 키워드 맵"
date: 2026-02-15T10:00:00
---

# 3000명 인사고과 AI 시험을 준비하며 정리한 키워드 맵

회사에서 갑자기 인사고과 시험 출제를 맡게 됐다. 그것도 3000명 규모의. SI 회사 개발 엔지니어들을 대상으로 AI 관련 문제를 내야 하는데, 솔직히 처음엔 막막했다. AI라는 게 워낙 넓고, 2025년부터 변화 속도가 미쳤기 때문이다.

출제를 잘하려면 일단 내가 전체 그림을 알아야 했다. 그래서 AI 개발에 관련된 키워드들을 학습 순서대로 정리해봤다. 이건 시험 출제용이기도 하지만, 교육자료의 뼈대로도 쓸 수 있을 거라 생각한다.
...
여기가 시험 출제하기 가장 좋은 영역이다. 명확한 정답이 있으니까.
...
모델 평가 쪽에서는 **과적합 vs 과소적합**, 교차 검증(K-Fold), 편향-분산 트레이드오프, 정규화(L1/L2/Dropout/Early Stopping)를 알아야 하고, 평가 지표로 Accuracy, Precision, Recall, F1-Score, AUC-ROC 개념은 확실히 잡아야 한다. 혼동 행렬(Confusion Matrix)에서 TP/TN/FP/FN을 해석하는 문제도 출제하기 좋다.
...
출제한다면 여기서 가장 많이 내게 될 거다. 2025-2026 기준 AI의 중심이니까.
...
SI 엔지니어들이 당장 실무에서 쓸 수 있는 영역이라 출제하면 반응이 좋을 것 같다.
...
기업 시험에서 빠지면 안 되는 영역. 특히 **EU AI Act**는 SI 회사라면 반드시 알아야 한다.
...
시험에 1~2문제는 최신 트렌드에서 내면 좋겠다는 생각이다.
...
:::tip 출제 비중 제안
SI 엔지니어 대상이라면 LLM+프롬프트+RAG에 약 40%, 에이전트+최신 트렌드에 20%, ML/DL 기초에 20%, MLOps+클라우드에 15%, 윤리/거버넌스에 5% 정도가 적절하다고 본다. SI 특성상 "AI를 시스템에 통합하는 관점"에 비중을 높이는 게 실무적으로 의미 있다.
:::

이 글이 완벽한 건 아니다. AI 분야는 매달 새로운 키워드가 쏟아지니까. 하지만 적어도 "어디서부터 봐야 하지?"라는 막막함을 줄여줄 수는 있을 거라 생각한다. 나도 이 정리를 기반으로 출제와 교육자료를 만들어갈 예정이다.
