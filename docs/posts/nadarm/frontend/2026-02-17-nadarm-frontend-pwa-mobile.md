---
title: "나닮 프론트엔드 — PWA 구현기와 iOS Safari와의 전쟁"
date: 2026-02-17T13:00:00
---

# 나닮 프론트엔드 — PWA 구현기와 iOS Safari와의 전쟁

나닮는 일기 앱이다. 매일 쓰는 서비스다. 그런데 매번 브라우저를 열고 주소를 입력해서 접속한다? 그건 일기장이 아니라 웹사이트다. 사용자가 홈 화면에서 아이콘 하나 톡 누르면 바로 일기를 쓸 수 있어야 한다. 그래서 PWA를 도입했고, 그 과정에서 iOS Safari와 길고 지루한 전쟁을 치르게 됐다.
...
그래서 나닮에서는 **3회 이상 방문한 사용자에게만** 설치 안내를 보여준다.
...
나닮에서는 `registerType: 'prompt'`를 선택했다. 이 설정의 의미는 새 SW가 준비되어도 자동으로 활성화하지 않고, 사용자에게 "업데이트 가능" 배너를 보여준 뒤 사용자가 직접 업데이트 버튼을 누르면 그때 `SKIP_WAITING` → 페이지 리로드를 수행하는 것이다.
...
나닮의 배경에는 이미지가 깔려 있고, 그 위에 반투명 배경 + blur 효과를 적용한 컨테이너가 올라간다. 꽤 예쁜 UI인데, iOS Safari에서 이 조합이 문제를 일으켰다.
...
나닮의 아이콘을 교체했다. 빌드하고 배포했다. 새 아이콘이 잘 나오는지 확인하려고 홈 화면을 봤는데 — 옛날 아이콘이 그대로 있다.
